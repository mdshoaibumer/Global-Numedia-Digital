          ...
          <Reveal variant="fade-right">
            <div className="lg:sticky lg:top-32 lg:self-start">
              ...
              <p className="mt-4 text-[15px] leading-relaxed text-ink-soft">
                Still wondering if we're a fit?{" "}
                <Link
                  to="/contact"
                  className="font-medium text-accent underline underline-offset-4 decoration-accent/30 hover:decoration-accent"
                >
                  Book a 30-min call
                </Link>{" "}
            ...
            {faqs.map((f, idx) => {
              ...
              return (
                <Reveal key={f.q} delay={idx * 0.05}>
                  <motion.div
                    className={`overflow-hidden rounded-2xl border transition-all duration-300 ${isOpen ? "border-accent/20 bg-accent/2 shadow-[0_8px_30px_-12px_rgba(120,119,198,0.1)]" : "border-border bg-card hover:border-accent/10"}`}
                    layout
                  >
                    <button
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          <p className="px-6 pb-6 text-[15px] leading-[1.75] text-ink-soft">
                            {f.a}
                          </p>
                        </motion.div>
                      )}
